...
def load_train_data_revise(PATH, seq_len, dim_num):
    ...
    #去除 nan 和 inf值
    matrix[torch.isinf(matrix) | torch.isnan(matrix)] = 0
    non_negetive_indices = torch.nonzero(matrix)

    _data = data[non_negetive_indices.flatten(), :]
    x_data = _data[:,0: seq_len * dim_num].reshape(_data.shape[0], seq_len, -1).permute(0,2,1)
    y_data = _data[:, -1].long().unsqueeze(1)
    #y_one_hot = one_hot_encode(y_data , 27)
    
    print('y_label.shape:', y_data.shape)
    print('x_data.shape :', x_data.shape)
    
    return x_data, y_data, non_negetive_indices

def load_train_data_4_relative_DOY(PATH, seq_len, dim_num):
    ...
    # apply relative DOY time series (DOY - DOY_first)
    _data = data[non_negetive_indices.flatten(), :]
    x_data = _data[:,0: seq_len * dim_num].reshape(_data.shape[0], seq_len, -1).permute(0,2,1) # X shape: [Batch, d_model, seq_len]
    doy_data, FB_data = x_data[:, 6, :], x_data[:, :6, :]
    first_doy = doy_data[:,  0].unsqueeze(1) - 1 
    doy_data_ = doy_data - first_doy
    x_data_ = torch.cat([FB_data, doy_data_.unsqueeze(1)], dim = 1)
    

    y_data = _data[:, -1].long().unsqueeze(1) # Y shape: [Batch, 1]
    y_mask = y_data > 366
    first_doy[y_mask] = 0
    y_data_ = y_data - first_doy
    #y_one_hot = one_hot_encode(y_data , 27)
    
    print('y_label.shape:', y_data_.shape)
    print('x_data.shape :', x_data_.shape)
    
    return x_data_, y_data_, first_doy
...
def Normalization2(x_feature):
    
    if not isinstance(x_feature, torch.Tensor):
        x_feature = torch.tensor(x_feature)
        
    feature_bands = x_feature[:, :6, :]  # shape: (Batch, 6, 27)

    # Calculate mean and std in dim2
    mean = torch.mean(feature_bands, dim=2, keepdim=True)  # shape: (Batch, 6, 1)
    std = torch.std(feature_bands, dim=2, keepdim=True)    # shape: (Batch, 6, 1)

    # Normalized
    feature_bands_norm = (feature_bands - mean) / std  # shape: (Batch, 6, 27)

    # Concat Feature bands and DOY
    doy_band = x_feature[:, 6:, :]  # shape: (Batch, 1, 27)
    doy_band = doy_band/367.0
    x_norm = torch.cat([feature_bands_norm, doy_band], dim=1)  # shape: (Batch, 7, 27)

    return x_norm

def Normalization3(x_feature):
    
    if not isinstance(x_feature, torch.Tensor):
        x_feature = torch.tensor(x_feature)
    
    band_num = x_feature.shape[1] - 1
    feature_bands = x_feature[:, :band_num, :]  # shape: (Batch, 6, 27)
...
def train_epoch(net, train_dl, device, optimizer, criterion):
    ...
    for step, (x,y) in enumerate(train_dl):
        ...
        output = net(x)
        print('--loss--output:', output.shape)
        print('--loss--y:', y.shape)
        print('output.squeeze():',output.squeeze().shape)
        print('y.cuda()[:, 0, :]:',y.cuda()[:, 0, :].shape)
...
def train_epoch2(net, train_dl, device, optimizer, criterion):
    ...
    for step, (x,y) in enumerate(train_dl):
        ...
        # print('output.squeeze():',output.squeeze().shape)
        # print('y.cuda()[:, 0, :]:',y.cuda()[:, 0, :].shape)
        loss = criterion(output.squeeze(), y.cuda()[:, 0, 0].float())
        loss.backward()
        optimizer.step()
...
def save_img(output_filename, reference_filename, array):
    ...
    #Get geo_transform params and projection
...
def get_result_revise(input_path, row, col, net, param_path, device, is_remove_band, target_band):

    #open image
    z = rxr.open_rasterio(input_path).values
    #print('z.shape', z.shape)
    
    zz = z[:z.shape[0], row[0]:row[1], col[0]:col[1]]    
    ...
    #------------revised
    x_feature = c.reshape(c.shape[0], 27 ,-1).permute(0,2,1)
    #print('d.shape:',x_feature.shape) # batch d_model seq_len
    ...
    #normalization
    x_norm = Normalization2(x_feature)
...
def get_result_revise_(input_path, row, col, net, param_path, device, is_remove_band, target_band):
...
def get_result_revise_1(input_path, row, col, net, param_path, is_remove_band, target_band, device):
    ...
    if is_remove_band:
        ...
        if target_band == 0:
            label = z[-1].reshape(1, z.shape[1], z.shape[2])
            z = np.delete(z, np.s_[target_band::7], axis=0)
            z = np.concatenate((z, label), axis = 0)
        elif target_band == 'vvvh':
            label = z[-1].reshape(1, z.shape[1], z.shape[2])
            z = np.delete(z, np.s_[0::7], axis=0)
            z = np.delete(z, np.s_[0::6], axis=0)
            z = np.concatenate((z, label), axis = 0)
        elif target_band == 'vvhh_except':
           # label = z[-1].reshape(1, z.shape[1], z.shape[2])
            z = np.delete(z, np.s_[2::7], axis=0)
            z = np.delete(z, np.s_[2::6], axis=0)
            z = np.delete(z, np.s_[2::5], axis=0)
            z = np.delete(z, np.s_[2::4], axis=0)
           # z = np.concatenate((z, label), axis = 0)
    ...
    #normalization
    x_norm = Normalization3(x_feature)
...
def get_result_revise_2(input_path, row, col, net, param_path, device):
